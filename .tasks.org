...
** Dates and schedules
   ...
   :ID: 45374501
   :END:
*** NEXT Date Parsing :date:parsing:
    :PROPERTIES:
    :ID: 78268550
    :END:
...
** Markdown rendering
   ...
   :ID: 48165813
   :END:
** AI Session Context & Tooling Progress
   :PROPERTIES:
   :ID: 96275729
   :END:
...
** TODO Derive Tool Input Schema from Go Struct using Reflection :jsonrpc:schema:input:
   ...
   :ID: 15951102
   :END:
   - [ ] Use the `reflect` package to traverse Go struct fields and tags.
   - [ ] Map Go types (string, int, bool, slices, etc.) to JSON Schema types.
   - [ ] Extract descriptions, required status, and constraints from struct tags (e.g., `jsonschema:"description=..."`).
   - [ ] Ensure nested structs are handled recursively to build complex input schemas.
